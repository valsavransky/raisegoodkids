...
- **Open question: does the "Skip" schedule-event category still earn its place?** A parent can already exclude an event from ever being scheduled by not selecting it at calendar-import time; tagging an already-added event "Skip" is a second, more confusing way to hide something (it stays stored, just disappears from the Schedule tab list). Leaning toward removing "Skip" as a category entirely, but not yet decided/actioned.
...
- **Settings page (Expected & Gigs / Account / Edit Profile) could use a real UX pass.** Not critical — flagged by the user while testing the new Edit Profile link, no specifics yet on what feels off. Revisit with concrete feedback once there's been more real use of the now-4-destination Settings area (it's grown from one screen to a tabbed one plus two linked-out screens fairly quickly).
