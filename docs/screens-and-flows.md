...
- **Google Calendar → Expected tasks UX — in progress.** Real OAuth (Google Cloud project, Android client, custom EAS dev-client build) and the practice-suggestion engine are built; the setup wizard's calendar screens now call the real Calendar API instead of mock data when signed in. Currently blocked on a live device bug: Google's authorization server rejects the sign-in request outright (Error 400: invalid_request, before any account picker even appears) — still being diagnosed, network access to Expo's/Google's own docs isn't available from the build session, so this is going slower than it should. The post-setup "Import from Google Calendar" screen (reachable from the Schedule tab) is still mock-only regardless.
