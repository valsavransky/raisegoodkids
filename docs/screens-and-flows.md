...
**Key elements:**
- Circular badge icon (heart-handshake for streaks, teal), confetti dots on unlock
...
- "Expected today" section header + streak indicator (heart-handshake icon, teal)
...
- Add/edit goal modal: once a name is typed, a guided two-step photo helper appears — "🔍 Search Google Images" (opens the device browser to a pre-filled image search, so the item can be seen and priced accurately) and "📎 Add photo" (picks the saved image from the phone's photo library and attaches it to the goal; shows a thumbnail with a Remove option once picked). The attached photo then shows on the goal card everywhere the goal appears (goal list, active goal). Deliberately manual, not automatic: pulling an image or price directly from a retailer would mean scraping outside their sanctioned API, which is against Amazon's (and most retailers') terms of service — a plain search link plus the person's own save-and-attach has no such restriction, since it's the same thing they'd do by hand. The photo is stored as a local file URI only (not uploaded anywhere), so it won't follow the goal across a cross-device account recovery.
...
- 2-column grid of badge tiles. Earned badges show in full category color (teal heart-handshake, magenta star, violet trending-up icon) with the date earned. Locked badges show grayed out (muted icon and text, disabled-fill circle) with a small lock icon and "Locked" instead of a date.
...
~~Active next workstream: Railway-backed persistence~~ **Done:** a small Express+Postgres API (`server/`) is live on Railway, with real accounts (email/password, JWT). Deliberately invisible on first launch — an account with throwaway random credentials is auto-created behind the scenes so `AppDataContext` starts backing up to the server immediately with zero signup friction; the app's whole local data blob is stored as JSONB per account, synced on every change, with server data adopted automatically on a fresh install/second device that has none locally yet. Verified end to end (signup, login, data round-trip, account-claim, sync) against both a local Postgres instance and the live Railway deploy. `parentAccountId` is still a hardcoded `'local-parent'` string in the client data model — real accounts exist server-side now, but nothing in `AppDataContext`'s own types reflects that yet.
- **Account-setup UX needs a real pass.** The "Secure your account" flow (Settings → Account tab) works but was built quickly to prove the backend end to end: it's just plain form fields, discoverability relies on a small dot on the settings gear, and there's no dedicated first-run screen or explanation of *why* a parent would bother. Revisit once there's a sense of whether the invisible-account model actually needs more prompting in practice (does anyone ever secure it unprompted?), and whether real 2FA belongs here at all rather than just email/password.
- **Cross-device data recovery is built but not yet proven on a real second device.** A "Log in" screen exists (setup wizard's first step → "Already set up Merit before?") so a secured account's data can be adopted on a device with no local data of its own — the server round-trip behind it is verified (curl tests against the live API), but the actual end-to-end phone test is still pending. The user's only spare device is an iPhone; ~~Google Calendar OAuth only has an Android client ID configured~~ **a real iOS OAuth client ID is now wired in** (`app.json`'s `extra.googleCalendar.iosClientId`), but real iPhone testing (of this and of Google Calendar import on iOS generally) is still blocked on an Apple Developer Program enrollment ($99/year, needed for a new EAS dev-client build with code signing) — the user doesn't have one yet and is deciding whether to enroll. A free alternative worth exploring if a Mac with Xcode is available: an iOS Simulator build doesn't need paid enrollment, though a simulator can't test real push notifications or camera-dependent features (not relevant to Merit today).

- ~~Content library for age/grade-appropriate Expected items and Gigs needs to be scoped as its own workstream~~ **In progress:** four grade bands now exist — K-2nd (`content-library-grade-k-2.md`, 10 Expected items, 13 Gigs, deliberately no Big Job tier and more "with help" framing than 3rd/4th — a younger band isn't just an easier version of the older one), 3rd/4th (`content-library-grade-3-4.md`, 11 Expected items, 19 Gigs), 5th/6th (`content-library-grade-5-6.md`, 12 Expected items, 20 Gigs — introduces the "Gig graduates to Expected" pattern, where a couple of tasks that were paid Gigs at younger bands become baseline unpaid responsibilities), and 7th/8th (`content-library-grade-7-8.md`, 12 Expected items, 21 Gigs — the most independent band so far, dropping the "help"/"supervised" qualifiers from most Gigs). All four gated by the same two household flags (yard, car — see `ChildProfile.hasYard`/`hasCar` and `SuggestedGig.ifApplicable`). **Still needs**: high school content (9th-12th, unauthored), and real-world testing/revision with families before treating any of it as final.
...
- ~~Bug: weekly Expected items reset daily instead of weekly~~ **Fixed:** completion checks (`src/utils/expectedItemStatus.ts`) are now period-aware — daily items still need a same-day completion, weekly items stay satisfied through the rest of the Sun-Sat week once done. The streak calculation (`src/utils/streak.ts`) uses the same check.
- ~~No persistence layer~~ **Partially done, superseded by the Railway workstream above:** `AppDataContext` persists to on-device storage (`@react-native-async-storage/async-storage`) as a single JSON blob, loaded on app start and saved on every change. Survives app restarts; still no backend/sync, and the setup wizard's own in-progress draft (`SetupContext`) is deliberately NOT persisted. A "Reset all data" action lives in the Expected & Gigs settings screen for testing.
- ~~Google Calendar → Expected tasks UX — blocked on a live device bug~~ **Working end to end:** real OAuth (Google Cloud project, Android client, custom EAS dev-client build), the practice-suggestion engine, and both calendar screens (setup wizard and the post-setup Schedule-tab version) now pull from the real Calendar API when signed in, falling back to mock data otherwise. Getting here required fixing, in order: the native redirect URI (had to match the app's package name, not an arbitrary custom scheme), enabling "Custom URI scheme" on the Android OAuth client in Cloud Console, forcing `prompt=consent` so Google doesn't silently reuse a stale narrower-scoped grant, and — the actual root cause — using the `calendar.readonly` scope instead of `calendar.events.readonly`, since the latter doesn't cover the CalendarList.list method needed just to show the calendar picker (confirmed via Google's own tokeninfo endpoint after everything else checked out correctly). Calendar import also now shows each recurring event's real cadence (parsed from its Google Calendar RRULE) instead of assuming weekly, and greys out/disables events already present in the schedule so re-importing can't create duplicates. A calendar event no longer auto-creates an Expected item on its own — only the practice-suggestion engine (offered when an event is tagged Practice/School, with a parent-adjustable daily/weekly cadence and duration) creates one, and only on explicit parent acceptance. iOS hasn't been tested (no iOS client ID configured yet) — same code path, untested device.
- ~~Hide the Android 3-button nav bar while inside the app~~ **Addressed, not via immersive mode:** rather than hiding the system nav bar, bottom-pinned buttons and every Modal-based bottom sheet now pad for the safe-area bottom inset so the nav bar can't cover them. Immersive/edge-to-edge mode (actually hiding the bar) is still a possible future option if the padding approach ever feels insufficient, but isn't planned.
- ~~Gig effort-tier dollar values were a hardcoded constant~~ **Done:** quick/medium/big_job dollar values are now parent-configurable in Manage Expected & Gigs (`gigEffortValues` in `AppDataContext`, defaults in `src/utils/gigValue.ts`), with visible save confirmation/validation.
- ~~Expected+Gigs setup was one long combined screen~~ **Done:** split into two focused wizard steps (`ExpectedSetupScreen`, `GigsSetupScreen`); wizard is now 4 steps instead of 3. The Manage Expected & Gigs settings screen is similarly split into Expected/Gigs tabs, with Gigs further split into a Gigs-list and Gig-values sub-tab.
~~Open question: does the "Skip" schedule-event category still earn its place?~~ **Resolved — removed.** Confirmed with the user it didn't (redundant with just not selecting an event at import time, and actively confusing on manual add — tagging a brand-new event to immediately hide it). Went further than just removing it: the whole category set was redesigned from School/Extracurricular/Practice/Skip to School/Sports/Extracurricular/Music/Other, with Sports gaining a practice-vs-game follow-up question and the suggestion engine becoming category-aware (see `src/data/practiceSuggestions.ts`).
~~Splash/start screen needs real content~~ **Done, and expanded further:** a real `WelcomeScreen` (new first screen in the setup wizard, before "Add a child") now carries the app's actual pitch — parent-facing, pulling language from the vision doc — instead of the wizard just starting cold with no context. The gap turned out to be bigger than just the splash moment once actually walked through screen by screen: the Expected and Gigs setup screens (which name the app's core concept) had *no* explanation of what those categories even mean, only practical mechanics ("suggestions are grade-specific..."). Both now open with a short parent-facing line explaining the concept, personalized with the child's name once it's been entered (step 1). The Goals screen's title/empty-state copy was tightened to match the same voice. Deliberately parent-facing throughout, not kid-voiced — the parent is alone during setup, the child isn't present yet (see the device/auth model decision above) — and gender-neutral ("they/their"), no pronoun field added.
- **Header navigation + personalization for parent and child** — the current `AppHeader` is the same on every screen (just a logo, the child's avatar emoji, and a settings gear — no names shown anywhere). Wants a real pass distinguishing parent-facing and child-facing context (per the dual-voice principle) and surfacing whose "mode" the app is in. Blocked on there being an actual place to set/edit a parent's name (see below).
~~No way to edit the child's profile (or set a parent's name/info) after initial setup~~ **Done:** a new `EditProfileScreen` (reachable via "Edit child & parent profile" in Settings) covers the same fields as the setup wizard, plus a real `parentName` field/functions in `AppDataContext` (there wasn't actually a public update function before, despite the old note here — only the internal state setter). Tested live on device. Unblocks header personalization, below — still not done.
~~Settings page (Expected & Gigs / Account / Edit Profile) could use a real UX pass~~ **Done:** redesigned from horizontal tabs under a "Close" text link into a grouped-row list (`SettingsScreen`) — the pattern most consumer apps use for settings — with a prominent Profile row at the top, Household/Account/Danger Zone sections below, and Expected Items/Gigs/Account each split into their own screen (`ExpectedItemsScreen`/`GigsScreen`/`AccountScreen`) reached by tapping a row, using a shared back-arrow header (`SettingsSubHeader`) instead of "Close." Wizard headers (`ScreenHeader`) got a matching pass: the Merit logo is now visible throughout setup (not just Welcome), "Step X of Y" text became a segmented progress bar, and a tappable child-profile chip (avatar + first name) jumps back to the Child Profile step once a name's been entered.
- **Household profile fields — done for yard/car/pet, open for anything more.** The child-profile setup step now asks yard/car/pet (with pet type + name), feeding the content-library gig filtering above and seeding a daily "Feed [pet]" Expected item plus, for a dog, a "Walk [pet]" gig. Further household context (siblings, who lives in the household, an existing allowance/savings starting balance for the Future Fund) was discussed but not built — open for a future pass if it'd meaningfully improve onboarding personalization.
- **Onboarding (screens 4-7) feels long and clunky.** The Expected/Gigs split above is a first, small cut at this; a fuller UX pass (which fields cause friction, whether the household questions belong on their own step, etc.) should wait for more real use.
- **Merit is single-child only**, despite `CLAUDE.md`'s "one or more child profiles" framing — `AppDataContext.childProfile` is one object, not an array. Real multi-child support (vs. just collecting descriptive facts about siblings) would be a genuinely large change touching nearly every screen; not scoped yet.
