...
- **No persistence layer.** All app data (`AppDataContext`) lives in plain React state — no AsyncStorage, no SQLite, no backend. Every full app reload wipes everything (child profile, completions, goals, Future Fund balance, badges). Worth prioritizing before much more feature work, since testing keeps getting interrupted by this.
