...
- **Cross-device data recovery is built but not yet proven on a real second device.** A "Log in" screen exists (setup wizard's first step → "Already set up Merit before?") so a secured account's data can be adopted on a device with no local data of its own — the server round-trip behind it is verified (curl tests against the live API), but the actual end-to-end phone test is still pending. Blocked on there being a second device to test with: the user's only spare is an iPhone, and Google Calendar OAuth only has an Android client ID configured (`app.json`'s `extra.googleCalendar.iosClientId` is empty) — the calendar-import screens would hit that same "not configured for this platform" mock-data fallback on iOS today. Both — adding a real iOS OAuth client and doing the cross-device recovery test — are queued as the next follow-up after the current Railway workstream wraps up.
