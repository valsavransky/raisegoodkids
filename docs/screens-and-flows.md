...
**Active next workstream: Railway-backed persistence.** `AppDataContext` currently persists only to on-device AsyncStorage (see below) — there is no backend, no real accounts (`parentAccountId` is a hardcoded `'local-parent'` string), and no way to recover data on a reinstall or a second device. The next planned body of work is standing up real persistence on Railway. Scope hasn't been finalized yet — options range from a full API+Postgres backend with real auth, to something narrower like periodic backup/sync of the existing local data blob — and should be settled before implementation starts.
