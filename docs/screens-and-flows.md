...
- **No way to edit the child's profile (or set a parent's name/info) after initial setup.** `AppDataContext.setChildProfile` exists, but no screen after onboarding calls it — the gear icon only reaches Expected & Gigs management. Needed both as a standalone gap (a typo'd name or wrong birthday is currently unfixable) and as a prerequisite for header personalization above.
