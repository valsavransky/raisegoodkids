...
- **Bug found, not yet fixed: weekly Expected items reset daily instead of weekly.** The "done today" check only compares against today's date regardless of an item's frequency, so a weekly item marked done on Monday incorrectly shows as not-done again on Tuesday — it should stay satisfied for the rest of that week and only reset once a new week starts. Daily items already reset correctly (that part needs no change). Fix: make the completion check period-aware — daily items compare to today's date as now, weekly items should match any completion within the current Sun-Sat (or Mon-Sun) week. This also affects the streak calculation (`src/utils/streak.ts`'s `isDayFullyCompleted`), which currently requires every item to have a completion on the exact day — it needs the same period-aware check so a weekly item doesn't need to be redone daily to keep the streak alive.
...
- **Google Calendar → Expected tasks UX** — still being thought through by the user before implementation. Current stubbed flow (mock calendar + events) turns a selected recurring event straight into a weekly Expected item; revisit once the user has a clearer UX direction (e.g. whether an Expected item should reference specific days via its ScheduleEvent, tying into the point above about weekly-item precision).
