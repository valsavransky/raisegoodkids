...
- ~~Content library for age/grade-appropriate Expected items and Gigs needs to be scoped as its own workstream~~ **Started:** an initial 3rd/4th grade set exists in `content-library-grade-3-4.md` (11 Expected items, 19 Gigs), now also gated by two household flags (yard, car — see `ChildProfile.hasYard`/`hasCar` and `SuggestedGig.ifApplicable`) so a family without a yard/car doesn't see irrelevant gig suggestions. **Still needs real expansion**: only 3rd/4th grade is authored. Backlog: expand to all grades K-12, grouped into elementary (K-5), middle (6-8), and high school (9-12) bands, plus real-world testing/revision with families before treating any of it as final.
