...
- **Header navigation + personalization for parent and child** — the current `AppHeader` is the same on every screen; wants a real pass distinguishing parent-facing and child-facing context (per the dual-voice principle) and surfacing whose "mode" the app is in.
- **Onboarding (screens 4-7) feels long and clunky** — needs a UX pass to shorten/streamline the setup wizard once there's been more real use to know which steps cause the most friction (related to the setup-forms UX pass noted above).
